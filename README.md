...
流程从一次只读扫描开始。技能先列出带层级的完整文件树，再精读清单、入口与主要配置，其余文件按需样读或跳过。它不追求读懂每一行实现，只求把项目理解到能写出一份合格文档的程度。

扫描的产物是一张证据图。技能据此按固定顺序填入有数据的章节，没有数据的章节整节跳过，不写 `N/A`，也不留占位。成稿之后还有七道门禁拦一道——证据、结构、语气、视觉、链接、无障碍与国际化，任何一项不过关就修复或删除，然后才写入文件。主要语言占 `README.md`，其他语言各占一个文件，切换栏双向可达。

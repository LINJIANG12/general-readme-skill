...
技能由一份路由入口与 15 个按需加载的参考文件组成，覆盖从扫描到交付的完整链路。证据图把每条断言绑定到真实来源；章节配方给出 20 类章节各自的写法与取舍，没有数据的章节整节跳过；视觉模板统一 Hero、目录、徽章、图表与折叠块，并约束 alt 文本与表头。多语言规范定义文件命名、双向切换栏与按区域映射的链接，交付前的质量门禁再校验证据、结构、语气、视觉、链接、无障碍与国际化。

...
General README Skill 把 README 撰写变成一次可校验的流程：先扫描仓库建立「断言 → 来源」证据图，再按固定的 20 个章节组合内容，最后跑七道门禁。任何无法追溯到真实文件的功能、命令或版本号都会被删除，而不是被弱化。技能以纯 Markdown 与 HTML 交付，可安装在 CodeBuddy、Claude Code、GitHub Copilot 与 Cursor 中，不引入额外运行时。

## 功能特性

| 特性 | 说明 |
|---|---|
| 无臆造内容 | 扫描不到的断言会被删除，README 只保留能验证的事实 |
| 结构一致 | 所有项目共用同一套章节顺序，读者不必重新适应版式 |
| 交付前自检 | 七道门禁在输出前逐项校验，失败项被修复或被明确报告 |
| 多语言与本地化 | 主要语言与次要语言并行，切换栏双向可达，链接按区域映射 |
| 可复现 | 同一仓库扫描得到同一套结构，输出稳定、可复查 |
| 零依赖 | 纯 Markdown 与 HTML，不需要 CLI、运行时或网络服务 |
...
技能由 `SKILL.md` 路由入口与 15 个参考文件组成。参考文件按需加载，只有在当前任务需要时才读取。

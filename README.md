...
生成文档遵循统一的 19 节单向结构。未探测到数据的章节整节隐去，绝不输出无意义的占位说明；成稿后由七道自动化质量门禁（证据、结构、语气、视觉、链接、无障碍与国际化）做终审交付。主要语言固定占位 `README.md`，次要语言通过双向顶栏无缝切换。
...
- **2 组合（Compose）** — 严格按固定 19 节顺序装配，无数据章节整节隐去，绝不产生空洞占位符。
- **3 校验（Verify）** — 运行 G1~G7 七道门禁，对任何未达标项执行修复或剔除。
- **4 输出（Output）** — 产出符合现代开源审美的 `README.md` 与多语言镜像文件。

<details>
<summary><b>点击展开：查看完整的 19 个固定章节定义与跳过规则</b></summary>
<br />
...
- **升级模式（Upgrade）**：当项目已有成型 README 时自动激活。AI 保持原文档中人工维护的独特手写段落，仅对陈旧、不一致的断言进行精准增量替换。

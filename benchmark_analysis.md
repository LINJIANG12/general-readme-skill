...
2. **统一章节库与排序原则**：不套用固定章节清单，改为按读者提问顺序（身份 → 演示 → 上手 → 机制 → 细节 → 运维 → 社区）选取与排序；扫描得不到数据的章节整节跳过。结构由项目与读者需求决定，而非项目类型。详见 `SKILL.md` → *Document Shape*。
3. **移除项目原型、成熟度分层与语气档案**：v2.0 的 8 种原型、3 级分层与 6 种语气已废弃，只保留一套排序逻辑与一套语气。
